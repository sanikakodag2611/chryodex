...
const today = new Date(); // Current date: 07:35 AM IST, October 01, 2025
...
export default function CityWise() {
  ...
  const fetchChartData = async () => {
    ...
    try {
      ...
      const res = await axios.get('http://127.0.0.1:8000/api/city-wise', {
        params,
      ...
      const cityList = [...new Set(data.map((d) => d.city))].sort();
      const series = [
        {
          name: 'Sales',
          data: cityList.map((name) =>
            Number(
              data
                .filter((x) => x.city === name)
      ...
      ].filter((s) => s.data.some((v) => v !== 0)); // Remove empty series

      console.log('Chart Series:', series); // Debug: Log processed series data

      const categories = cityList.map((name) => name || 'Unknown');
      ...
      setChartData({
        ...
        options: {
          ...
          xaxis: {
            categories,
            title: { text: 'City', style: { fontWeight: 600, fontSize: '14px' } },
            labels: {
              rotate: -45,
  ...
  const fetchTableData = async () => {
    ...
    try {
      ...
      const res = await axios.get('http://127.0.0.1:8000/api/city-details', {
        params,
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'CityWiseReport');
    const excelBuffer = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    const data = new Blob([excelBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
    saveAs(data, `CityWiseReport_${format(today, 'yyyyMMdd_HHmmss')}.xlsx`);
  };
  ...
  return (
    <MainCard title="City Wise Report">
      <style>
        {`
      ...
      <div className="scroll-container">
        <div className="header-section">
          <div className="header-content">
            ...
            <p className="date-time">City Wise Sales Report</p>
          </div>
        ...
        <div style={{ padding: '0 10px' }} ref={tableRef}>
          <h3 style={{ textAlign: 'center' }}>City Details</h3>
          <table style={{ border: '1px solid #ccc' }}>
            <thead>
              <tr style={{ backgroundColor: '#f0f0f0' }}>
                <th style={{ border: '1px solid #ccc', padding: '5px', textAlign: 'center' }}>City</th>
