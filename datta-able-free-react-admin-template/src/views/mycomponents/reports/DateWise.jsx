...
const today = new Date(); // 06:23 PM IST, September 25, 2025
...
export default function DateWise() {
  ...
  const fetchChartData = async () => {
    try {
      ...
      const res = await axios.get('http://127.0.0.1:8000/api/date-wise', {
        params,
      ...
      const dateList = [...new Set(data.map((d) => d.date))].sort((a, b) => new Date(b) - new Date(a));
      const series = [
        {
          name: 'Sales',
          data: dateList.map((d) =>
            Number(data.filter((x) => x.date === d).reduce((a, b) => a + (Number(b.sales) || 0), 0).toFixed(2))
          ),
        },
        {
          name: 'Profit',
          data: dateList.map((d) =>
            Number(data.filter((x) => x.date === d).reduce((a, b) => a + (Number(b.profit) || 0), 0).toFixed(2))
          ),
        },
      ];
      const categories = dateList.map((d) => format(new Date(d), 'dd-MM-yyyy'));
      ...
      setChartData({
        ...
        options: {
          ...
          xaxis: {
            categories,
            title: { text: 'Date', style: { fontWeight: 600, fontSize: '14px' } },
            labels: {
              rotate: -45,
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'DateWiseReport');
    const excelBuffer = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    const data = new Blob([excelBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
    saveAs(data, `DateWiseReport_${format(today, 'yyyyMMdd_HHmmss')}.xlsx`);
  };
  ...
  return (
    <MainCard title="Date Wise Report">
      <style>
        {`
        ...
        <div className="header-section">
          <div className="header-content">
            ...
            <h4 className="company-name">Chryodex Construction chemical PVT.LTD</h4>
            <p className="date-time">Date: {format(today, 'dd-MM-yyyy hh:mm a')}</p>
            <p className="date-time">Date wise sales Report</p>
          </div>
        ...
        <div style={{ padding: '0 10px' }} ref={tableRef}>
          <h3 style={{ textAlign: 'center' }}>Sales Details</h3>
          <table style={{ border: '1px solid #ccc' }}>
            <thead>
              <tr style={{ backgroundColor: '#f0f0f0' }}>
                <th style={{ border: '1px solid #ccc', padding: '5px', textAlign: 'center' }}>Date</th>
